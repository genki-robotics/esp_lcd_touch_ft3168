...
static bool esp_lcd_touch_ft3168_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y,
...
static esp_err_t touch_ft3168_i2c_read(esp_lcd_touch_handle_t tp, uint8_t reg, uint8_t *data,
                                       uint8_t len);
...
esp_err_t esp_lcd_touch_new_i2c_ft3168(const esp_lcd_panel_io_handle_t io,
  ...
  esp_lcd_touch_handle_t esp_lcd_touch_ft3168 = static_cast<esp_lcd_touch_handle_t>(
  ...
  if (esp_lcd_touch_ft3168->config.int_gpio_num != GPIO_NUM_NC) {
    const gpio_config_t int_gpio_config = {
        ...
        .intr_type = (esp_lcd_touch_ft3168->config.levels.interrupt ? GPIO_INTR_POSEDGE
                                                                    : GPIO_INTR_NEGEDGE)};
    ...
    if (esp_lcd_touch_ft3168->config.interrupt_callback) {
      esp_lcd_touch_register_interrupt_callback(esp_lcd_touch_ft3168,
                                                esp_lcd_touch_ft3168->config.interrupt_callback);
    }
  }
  ...
  if (esp_lcd_touch_ft3168->config.rst_gpio_num != GPIO_NUM_NC) {
    const gpio_config_t rst_gpio_config = {
        .pin_bit_mask = BIT64(esp_lcd_touch_ft3168->config.rst_gpio_num), .mode = GPIO_MODE_OUTPUT};
